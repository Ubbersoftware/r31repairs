...
export async function getActiveServices(): Promise<Service[]> {
  const snap = await getAdminDb().collection('r31_services').where('active', '==', true).get()
  return snap.docs.map((d) => toService(d.id, d.data())).sort((a, b) => a.sortOrder - b.sortOrder)
}

export async function getServiceBySlug(slug: string): Promise<Service | null> {
  const doc = await getAdminDb().collection('r31_services').doc(slug).get()
  if (!doc.exists) return null
  const s = toService(doc.id, doc.data()!)
  return s.active ? s : null
}

export async function getActiveModels(): Promise<PhoneModel[]> {
  const snap = await getAdminDb().collection('r31_phoneModels').where('active', '==', true).get()
...
export async function getPriceMatrix(): Promise<PriceDoc[]> {
  const snap = await getAdminDb().collection('r31_prices').get()
  return snap.docs.map((d) => toPriceDoc(d.data()))
}

export async function getActiveFaqs(): Promise<Faq[]> {
  const snap = await getAdminDb().collection('r31_faqs').where('active', '==', true).get()
  return snap.docs.map((d) => toFaq(d.id, d.data())).sort((a, b) => a.sortOrder - b.sortOrder)
}

export async function getAllFaqs(): Promise<Faq[]> {
  const snap = await getAdminDb().collection('r31_faqs').get()
  return snap.docs.map((d) => toFaq(d.id, d.data())).sort((a, b) => a.sortOrder - b.sortOrder)
}
